function EHT_motion_tracker(template_path, data_path, result_path, perform_annotation, config_file, force_reannotation, folder_filter)
...
for i = 1:length(contents)
    ...
    % If folder_filter is provided, skip folders not in the filter
    if nargin >= 7 && ~isempty(folder_filter)
        if ~ismember(folder_name, folder_filter)
            continue;
        end
    end
...
write_results_v2(results, result_path, wells, data_path, config);
...
for i = 1:length(well_names)
    ...
    imshow(uint8(img), []);
    if config.show_annotation_guidance
        title(sprintf('Well %s: Drag/resize box for FIRST post (Left). Double-click inside to confirm.', well_id), 'FontSize', 12);
    end

    [template1, x_offset] = get_valid_template(img, config, 'First', well_id, 10);

    if ~isempty(template1)
        if config.show_annotation_guidance
            title(sprintf('Well %s: Drag/resize box for SECOND post (Right). Double-click to confirm.', well_id), 'FontSize', 12);
        end

        [template2, ~] = get_valid_template(img, config, 'Second', well_id, size(img, 2) - config.template_width - 10);

        if ~isempty(template2)
...
metadata_struct.Config = config;
metadata_struct.ExportDate = timestamp_str;
metadata_struct.Wells = wells;
...
function [template, final_x] = get_valid_template(img, config, post_name, well_id, default_x)
    initial_y = round(size(img,1)/2 - config.template_height/2);
    valid = false;
    template = [];
    final_x = 0;
    
    while ~valid
        h = imrect(gca, [default_x, initial_y, config.template_width, config.template_height]);
        setResizable(h, true);
        wait(h);
        pos = getPosition(h);
        ...
        err_msg = '';
        if w < config.min_template_width || h_box < config.min_template_height
            err_msg = sprintf('Size too small (%dx%d). Min: %dx%d.', w, h_box, config.min_template_width, config.min_template_height);
        elseif w > config.max_template_width || h_box > config.max_template_height
            err_msg = sprintf('Size too large (%dx%d). Max: %dx%d.', w, h_box, config.max_template_width, config.max_template_height);
        elseif x1 <= 1 || y1 <= 1 || x2 >= size(img, 2) || y2 >= size(img, 1)
            err_msg = 'Region touching image boundary. This causes edge-artifacts in tracking.';
        end
        
        if ~isempty(err_msg)
            warndlg(err_msg, 'Invalid Selection');
            delete(h);
        else
            valid = true;
            template = img(max(1, y1):min(size(img,1), y2), max(1, x1):min(size(img,2), x2));
            final_x = x1;
            delete(h);
        end
    end
