...
- A PC connected to the mechanical stage and the Arduino. Using Micromanager as the hardware controller and the script EHT-acquire, the PC automates multiwell acquisition.
- Post-acquisition video analysis by EHT-motion to yield EHT post positions over time.
- EHT-analyze, which analyzes post position vs time to calculate contraction and relaxation kinetics and force.
...
9. **Configure Fiji to analyze post positions**

In Fiji, install the multi-template matching plugin as described here: <https://multi-template-matching.github.io/Multi-Template-Matching/>

Install the Fiji plugin EHT-Motion, which analyzes EHT video images and yields a table of position vs time.
